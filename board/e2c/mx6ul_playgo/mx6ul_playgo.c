...
#define CONFIG_MX6UL_PLAYGO_EMMC

/* PlayGo - Changed I2C1 pin allocation */
...
/* SC100 - Changed UART1 pin allocation */
...
/* USDHC1 Main boot device. Can be SD OR eMMC depending on board hardware config */
#if defined(CONFIG_MX6UL_PLAYGO_EMMC)
...
static void eim_clk_setup(void)
{
	...
	cscmr1 |= (3 << MXC_CCM_CSCMR1_ACLK_EMI_SLOW_PODF_OFFSET);
	...
	writel(0x0191000f,&weim_regs->cs0gcr1);
	writel(0x00001200,&weim_regs->cs0gcr2);
	writel(0x02001000,&weim_regs->cs0rcr1);
	writel(0x0000c008,&weim_regs->cs0rcr2);
	writel(0x0204a38e,&weim_regs->cs0wcr1);
	writel(0x00000000,&weim_regs->cs0wcr2);

	wcr = readl(&weim_regs->wcr);
	wcr = wcr | 0x9;
	writel(wcr,&weim_regs->wcr);

	/* Now do dummy read on EIM to start clk */
...
#define USDHC1_CD_GPIO	IMX_GPIO_NR(5, 8)
#define USDHC1_RST_GPIO	IMX_GPIO_NR(2, 8)
...
/*	Add any resetting of device here */
...
int board_late_init(void)
{
	...
	env_set("board_name", "PlayGo");
	env_set("board_rev", "0a");
#endif
